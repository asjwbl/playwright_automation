...
export class TableComponent extends BasicComponent {
  ...
  async getCellText(rowIndex: number, colIndex: number): Promise<string> {
    const cell = this.locator.locator(
      `tbody tr:nth-of-type(${rowIndex}) td:nth-of-type(${colIndex})`
    );
  ...
  async verifyCellText(
    ...
    if (cellText.trim() !== expectedText.trim()) {
      throw new Error(
        `Expected cell at row ${rowIndex}, col ${colIndex} to contain "${expectedText}", but got "${cellText}"`
      );
    }
  }
