...
export class TextAreaComponent extends BasicComponent {
    ...
    async enterText(text: string): Promise<void> {
        try {
            ...
            if (error instanceof Error) {
                console.error(`Error while entering text in the textarea: ${error.message}`);
            } else {
    ...
    async getValue(): Promise<string | null> {
        try {
            ...
            if (error instanceof Error) {
                console.error(`Error while retrieving textarea value: ${error.message}`);
            } else {
                console.error(`An unexpected error occurred: ${error}`);
            }
            throw error;
    ...
    async appendText(text: string): Promise<void> {
        try {
            ...
            if (error instanceof Error) {
                console.error(`Error while appending text to the textarea: ${error.message}`);
            } else {
                console.error(`An unexpected error occurred: ${error}`);
            }
            throw error;
