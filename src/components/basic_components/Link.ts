...
export class LinkComponent extends BasicComponent {
    ...
    async getHref(): Promise<string> {
        try {
            ...
            console.error(`Error retrieving the href attribute of the link: ${(error as Error).message}`);
            throw error;
        }
    }
    ...
    async rightClick(): Promise<void> {
        try {
            ...
            console.error(`Error right-clicking the link: ${(error as Error).message}`);
            throw error;
        }
    }
}
