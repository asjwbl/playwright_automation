...
export class ToastComponent extends BasicComponent {
    ...
    async dismiss(): Promise<void> {
        try {
            ...
            console.error(`Error dismissing the toast notification: ${(error as Error).message}`);
            throw error;
        }
    }
    ...
    async isVisibleWithMessage(message: string): Promise<boolean> {
        try {
            ...
            console.error(`Error checking if the toast notification is visible with message: ${(error as Error).message}`);
            return false;
        }
    }
    ...
    async hasError(): Promise<boolean> {
        try {
            const errorToast = await this.page.locator('[data-testid="toast-error"]').isVisible();
            return errorToast;
        } catch (error: unknown) {
            console.error(`Error checking for toast error: ${(error as Error).message}`);
            return false;
        }
    }
    ...
    async waitForDismiss(timeout: number = 5000): Promise<void> {
        try {
            ...
            console.error(`Error waiting for the toast notification to disappear: ${(error as Error).message}`);
            throw error;
        }
    }
}
